...
class SeerrClient(BaseArrClient):
    """Client for interacting with Seerr API."""

    def __init__(self, base_url: str, api_key: str, timeout: int = 30, max_retries: int = 3):
        """Initialize Seerr client (uses v1 API)."""
        super().__init__(base_url, api_key, timeout, max_retries)
        self._api_version = "v1"  # Seerr uses v1
        self._csrf_token: Optional[str] = None

    async def _ensure_csrf(self) -> str:
        """Fetch a CSRF token from the Overseerr homepage if not already cached."""
        if self._csrf_token:
            return self._csrf_token
        base = self.base_url.rstrip("/api/v1").rstrip("/")
        resp = await self.client.get(f"{base}/", follow_redirects=True)
    ...
    async def post(self, endpoint: str, json: Optional[dict] = None) -> Any:
        """POST with Overseerr CSRF token header."""
        csrf = await self._ensure_csrf()
        url = self._build_url(endpoint)
        headers = {**self._get_headers(), "X-CSRF-Token": csrf, "Content-Type": "application/json"}
        response = await self.client.post(url, headers=headers, json=json or {})
        if response.status_code >= 400:
            from .base import ArrClientError
            raise ArrClientError(f"Seerr: HTTP {response.status_code} - {response.text}")
    ...
    async def get_system_status(self) -> dict[str, Any]:
        """Get Seerr status (endpoint differs from other arr services)."""
